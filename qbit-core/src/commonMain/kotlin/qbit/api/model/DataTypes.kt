...
sealed class DataType<out T : Any> {
    ...
    companion object {
        ...
        fun ofCode(code: Byte): DataType<*>? = when(code.toUByte()) {
            ...
            in counterRange -> ofCode((code.toUByte() - counterRange.first).toByte())?.counter()
    ...
    fun isCounter(): Boolean = code.toUByte() in counterRange
...
data class QCounter<out I : Any>(val primitiveType: DataType<I>) : DataType<I>() {
    ...
    override val code = (counterRange.first.toByte() + primitiveType.code).toByte()
